...
# Disable opcache
if [ ! -z "$OPcache" ]; then
 sed -i 's#zend_extension=opcache#;zend_extension=opcache#g' /etc/php/5.6/cli/conf.d/10-opcache.ini
fi
...
# Prevent config files from being filled to infinity by force of stop and restart the container
lastlinephpconf="$(grep "." /etc/php/5.6/fpm/php.ini | tail -1)"
if [[ $lastlinephpconf == *"php_flag[display_errors]"* ]]; then
 sed -i '$ d' /etc/php/5.6/fpm/php.ini
fi
...
if [[ "$HIDE_NGINX_HEADERS" == "0" ]] ; then
 ...
 sed -i "s/expose_php = On/expose_php = Off/g" /etc/php/5.6/fpm/php.ini
fi
...
if [ ! -z "$PHP_ERRORS_STDERR" ]; then
  echo "log_errors = On" >> /etc/php/5.6/cli/conf.d/docker-vars.ini
  echo "error_log = /dev/stderr" >> /etc/php/5.6/cli/conf.d/docker-vars.ini
fi

# Increase the memory_limit
if [ ! -z "$PHP_MEM_LIMIT" ]; then
 sed -i "s/memory_limit = 128M/memory_limit = ${PHP_MEM_LIMIT}M/g" /etc/php/5.6/cli/conf.d/docker-vars.ini
fi


# Increase PHP-FPM WORKER
if [ ! -z "$FPM_MAX_WORKER" ]; then
 sed -i "s/pm.max_children = 5/pm.max_children = ${FPM_MAX_WORKER}/g" /etc/php/5.6/fpm/pool.d/www.conf
fi

if [ ! -z "$FPM_START_WORKER" ]; then
 sed -i "s/pm.start_servers = 2/pm.start_servers = ${FPM_START_WORKER}/g" /etc/php/5.6/fpm/pool.d/www.conf
fi

if [ ! -z "$FPM_SPARE_MIN_WORKER" ]; then
 sed -i "s/pm.min_spare_servers = 1/pm.min_spare_servers = ${FPM_SPARE_MIN_WORKER}/g" /etc/php/5.6/fpm/pool.d/www.conf
fi

if [ ! -z "$FPM_SPARE_MAX_WORKER" ]; then
 sed -i "s/pm.max_spare_servers = 3/pm.max_spare_servers = ${FPM_SPARE_MAX_WORKER}/g" /etc/php/5.6/fpm/pool.d/www.conf
fi


# Increase the post_max_size
if [ ! -z "$PHP_POST_MAX_SIZE" ]; then
 sed -i "s/post_max_size = 100M/post_max_size = ${PHP_POST_MAX_SIZE}M/g" /etc/php/5.6/cli/conf.d/docker-vars.ini
fi

# Increase the upload_max_filesize
if [ ! -z "$PHP_UPLOAD_MAX_FILESIZE" ]; then
 sed -i "s/upload_max_filesize = 100M/upload_max_filesize= ${PHP_UPLOAD_MAX_FILESIZE}M/g" /etc/php/5.6/cli/conf.d/docker-vars.ini
fi

# Enable xdebug
XdebugFile='/etc/php/5.6/cli/conf.d/20-xdebug.ini'
